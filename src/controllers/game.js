// --- Registrar scores finales en el backend ---
async function registrarScoresFinales(jugadores) {
...
// --- Evento al confirmar finalizar juego ---
document.addEventListener("DOMContentLoaded", function () {
  // Poblar window.jugadores desde localStorage si no existe
  if (!window.jugadores) {
    const playersData = localStorage.getItem("playersData");
    if (playersData) {
      // Asume que cada jugador tiene username, country, color
      // Inicializa score en 0 si no existe
      window.jugadores = JSON.parse(playersData).map((p) => ({
        nombre: p.username,
        score: p.score || 0,
  ...
  if (confirmEndGameBtn) {
    confirmEndGameBtn.addEventListener("click", async function () {
      // window.jugadores debe tener [{nombre, score, country_code}, ...]
      const jugadores = window.jugadores || [];
      if (jugadores.length > 0) {
        await registrarScoresFinales(jugadores);
      }
      // Opcional: mostrar modal de estadísticas finales, redirigir, etc.
...
class Game {
    ...
    if (playersList) {
      playersList.innerHTML = this.players
        .map((player) => {
          ...
          const mortgageDebt = Array.from(
            player.mortgagedProperties.values()
          ).reduce((sum, value) => sum + Math.floor(value * 1.1), 0);

          return `
                    <div class="player-info-card ${
                      player === this.getCurrentPlayer() ? "active" : ""
                    }" 
                         style="border-left: 4px solid ${player.getColorCode()}"
                         onclick="window.gameInstance.showPlayerPropertiesModal('${
                           player.username
                         }')"
                        ...
                        <div class="player-name">
                            ...
                            ${
                              mortgagedCount > 0
                                ? `<span class="mortgage-indicator" title="${mortgagedCount} propiedades hipotecadas">🏦${mortgagedCount}</span>`
                                : ""
                            }
                        </div>
                        <div class="player-details">
                            ...
                            <div class="${
                              player.money < 100 ? "text-danger" : ""
                            }">💰 $${player.money.toLocaleString()}</div>
                            <div>🏠 Propiedades: ${
                              player.properties.length +
                              player.railroads.length +
                              player.utilities.length
                            }</div>
                            <div>🏗️ Casas: ${player.houses} | 🏨 Hoteles: ${
            player.hotels
          }</div>
                            ${
                              mortgageDebt > 0
                                ? `<div class="text-warning">🔒 Deuda: $${mortgageDebt.toLocaleString()}</div>`
                                : ""
                            }
  ...
  startGame() {
    ...
    console.log(
      "Game started! Current player:",
      this.getCurrentPlayer().username
    );

    // Mostrar mensaje inicial
    this.showGameMessage(
      `¡Juego iniciado! Es el turno de ${this.getCurrentPlayer().username}`
    );
  ...
  getDiceSum() {
    ...
    if (sumLabel && sumLabel.textContent) {
      ...
      if (match) {
        ...
        if (sum > 0 && sum <= 12) {
          // Validar rango válido para dados
    ...
    if (die1 && die2 && die1.textContent && die2.textContent) {
      ...
      if (sum >= 2 && sum <= 12) {
        // Rango válido para suma de dos dados
        console.log(
          `Calculated dice sum: ${die1Value} + ${die2Value} = ${sum}`
        );
        return sum;
      }
    }
  ...
  handlePropertySpace(player, spaceInfo) {
    ...
    if (!owner) {
      ...
      // Es propiedad propia
      this.showGameMessage(
        `${player.username} está en su propia propiedad: ${spaceInfo.name}`
      );
      ...
      body: `
                <div class="property-purchase-dialog">
                    ...
                    <div class="property-details">
                        ...
                        <p><strong>Renta base:</strong> $${
                          property.rent
                            ? property.rent.base || property.rent[1]
                            : "N/A"
                        }</p>
                        <p><strong>Tu dinero:</strong> $${player.money}</p>
                        ${
                          property.color
                            ? `<p><strong>Color:</strong> ${property.color}</p>`
                            : ""
                        }
                    </div>
                    ${
        ...
        ? [
            {
              ...
              action: () => {
                if (player.buyProperty(property)) {
                  this.showGameMessage(
                    `${player.username} compró ${property.name} por $${property.price}`
                  );
                  this.updatePlayerInfoPanel();
                }
              },
            },
            {
              ...
              action: () => {
                this.showGameMessage(
                  `${player.username} decidió no comprar ${property.name}`
                );
              },
            },
          ]
  ...
  payRent(player, owner, property) {
    const rentAmount = owner.calculateRent(
      owner.getProperty(property.id),
    ...
    if (rentAmount > 0) {
      ...
      this.showGameMessage(
        `${player.username} pagó $${rentAmount} de renta a ${owner.username} por ${property.name}`
      );
      this.updatePlayerInfoPanel();
      ...
      if (window.jugadores) {
        const jugador = window.jugadores.find(
          (j) => j.nombre === player.username
        );
        if (jugador) {
          jugador.score = player.money;
        }
      }
    }
  }
  ...
  showPropertyManagementDialog(player, property) {
    ...
    const canBuildHouse = player.canBuildHouse(
      property.id,
      this.getAllProperties()
    );
    const canBuildHotel = player.canBuildHotel(
      property.id,
      this.getAllProperties()
    );
                ...
                <div class="property-management-dialog">
                    ...
                    <div class="property-details">
                        ...
                        <p><strong>Hotel:</strong> ${
                          ownedProperty.hotel ? "Sí" : "No"
                        }</p>
                        <p><strong>Renta actual:</strong> $${player.calculateRent(
                          ownedProperty,
                          this.getAllProperties()
                        )}</p>
                        <p><strong>Tu dinero:</strong> $${player.money}</p>
          ...
          ? [
              {
                ...
                action: () => {
                  if (player.buildHouse(property.id, this.getAllProperties())) {
                    this.showGameMessage(
                      `${player.username} construyó una casa en ${property.name}`
                    );
                    this.updatePlayerInfoPanel();
                  }
                },
          ...
          ? [
              {
                ...
                action: () => {
                  if (player.buildHotel(property.id, this.getAllProperties())) {
                    this.showGameMessage(
                      `${player.username} construyó un hotel en ${property.name}`
                    );
                    this.updatePlayerInfoPanel();
                  }
                },
  ...
  handleTaxSpace(player, spaceInfo) {
    ...
    this.showGameMessage(
      `${player.username} pagó $${taxAmount} en impuestos en ${spaceInfo.name}`
    );
    this.updatePlayerInfoPanel();
  ...
  handleSpecialSpace(player, spaceInfo) {
    switch (spaceInfo.name) {
      ...
      case "Parqueo Gratis":
        this.showGameMessage(
          `${player.username} está descansando en el Parqueo Gratis`
        );
        break;
      case "Cárcel / Solo de visita":
        if (!player.isInJail) {
          this.showGameMessage(
            `${player.username} está de visita en la Cárcel`
          );
        }
        break;
      case "Ve a la Cárcel":
  ...
  handleChanceSpace(player) {
    ...
    const randomCard =
      this.boardData.chance[
        Math.floor(Math.random() * this.boardData.chance.length)
      ];
  ...
  handleCommunityChestSpace(player) {
    ...
    const randomCard =
      this.boardData.community_chest[
        Math.floor(Math.random() * this.boardData.community_chest.length)
      ];
  ...
  executeCardAction(player, card, cardType) {
    ...
    if (card.action) {
      if (card.action.money) {
        ...
        this.showGameMessage(
          `${player.username} ${action} $${Math.abs(card.action.money)} - ${
            card.description
          }`
        );
      }
                ...
                <div class="card-modal text-center">
                    ...
                    ${
                      card.action?.money
                        ? `
                        <div class="amount ${
                          card.action.money > 0 ? "text-success" : "text-danger"
                        }">
                            ${card.action.money > 0 ? "+" : ""}$${
                            card.action.money
                          }
                ...
                <div class="player-properties-modal">
                    <div class="player-summary mb-4">
                            ...
                            <div class="col-3">
                                <div class="summary-item">
                                    <div class="summary-value text-warning">${
                                      player.mortgagedProperties.size
                                    }</div>
                        ...
                        <div class="row text-center mt-3">
                            ...
                            <div class="col-4">
                                <div class="summary-item">
                                    <div class="summary-value text-danger">$${Array.from(
                                      player.mortgagedProperties.values()
                                    )
                                      .reduce(
                                        (sum, value) =>
                                          sum + Math.floor(value * 1.1),
                                        0
                                      )
                                      .toLocaleString()}</div>
  ...
  calculatePlayerNetWorth(player) {
    ...
    const mortgageDebt = Array.from(player.mortgagedProperties.values()).reduce(
              ...
              .map(
                ...
                <div class="color-group mb-3">
                    <div class="color-header d-flex align-items-center mb-2">
                        <div class="color-indicator me-2" style="background-color: ${
                          colorStyles[color] || "#666"
                        }"></div>
                        <strong>${colorNames[color] || color} (${
                  properties.length
                })</strong>
                        ${
                          this.hasColorMonopoly(color, properties)
                            ? '<span class="badge bg-success ms-2">Monopolio</span>'
                            : ""
                        }
                    </div>
                        ...
                        ${properties
                          .map(
                            ...
                            <div class="property-item">
                                ...
                                <div class="property-details">
                                    <span class="rent-info">Renta: $${this.calculateCurrentRent(
                                      property
                                    )}</span>
                                    ${
                                      property.houses > 0
                                        ? `<span class="houses-info">${property.houses} casas</span>`
                                        : ""
                                    }
                                    ${
                                      property.hotel
                                        ? '<span class="hotel-info">🏨 Hotel</span>'
                                        : ""
                                    }
                                </div>
                            </div>
                        ...
                        ${player.railroads
                          .map(
                            ...
                            <div class="property-item">
                                ...
                                <div class="property-details">
                                    <span class="rent-info">Renta: $${
                                      railroad.rent[player.railroads.length]
                                    }</span>
                                </div>
                            </div>
                        `
    ...
    if (player.utilities.length > 0) {
      html += `
                <div class="utilities-section mb-3">
                    <h6 class="mb-2">⚡ Servicios Públicos (${
                      player.utilities.length
                    })</h6>
                    <div class="properties-list">
                        ${player.utilities
                          .map(
                            ...
                            <div class="property-item">
                                ...
                                <div class="property-details">
                                    <span class="rent-info">Renta: Dados × ${
                                      player.utilities.length === 1 ? "4" : "10"
                                    }</span>
                                </div>
                            </div>
                        `
  ...
  calculateCurrentRent(property) {
    if (property.hotel) {
      ...
      const hasMonopoly = this.hasColorMonopoly(
        property.color,
        this.players
          .find((p) => p.properties.some((pr) => pr.id === property.id))
          ?.properties.filter((p) => p.color === property.color) || []
      );
                ...
                <div class="winner-announcement text-center">
                    ...
                    <div class="winner-stats">
                        ...
                        <p><strong>Propiedades:</strong> ${
                          ...
                          winner.railroads.length +
                          winner.utilities.length
                        }</p>
  ...
  showEndGameConfirmation() {
    const modal = new bootstrap.Modal(
      document.getElementById("endGameConfirmModal")
    );
    modal.show();
  ...
  saveGameHistory(stats) {
    try {
      let gameHistory = JSON.parse(
        localStorage.getItem("monopolyGameHistory") || "[]"
      );
  ...
  calculateGameStats() {
    const totalTurns =
      ...
      this.players.indexOf(this.getCurrentPlayer());
  ...
  showGameStats(stats) {
                              ...
                              player.finalPosition === 1
                                ...
                                <td>
                                    <div class="d-flex align-items-center">
                                        <div class="player-color-indicator me-2" 
                                             style="width: 15px; height: 15px; border-radius: 50%; background-color: ${this.getPlayerColorCode(
                                               player.color
                                             )}"></div>
                                ...
                                <td class="${
                                  player.money < 0
                                    ? "text-danger"
                                    : "text-success"
                                }">$${player.money.toLocaleString()}</td>
                                ...
                                <td>
                                    ${
                                      player.money < 0
                                        ? '<span class="badge bg-danger">Bancarrota</span>'
                                        : player.isInJail
                                        ? '<span class="badge bg-warning">En Cárcel</span>'
                ...
                <div class="row">
                    <div class="col-md-6">
                        <p><strong>Ganador:</strong> ${
                          stats.players[0].username
                        } (${stats.players[0].country})</p>
                        <p><strong>Dinero del Ganador:</strong> $${stats.players[0].money.toLocaleString()}</p>
                    </div>
                    <div class="col-md-6">
                        <p><strong>Propiedades del Ganador:</strong> ${
                          stats.players[0].properties
                        }</p>
    ...
    // Mostrar el modal
    const modal = new bootstrap.Modal(
      document.getElementById("gameStatsModal")
    );
    modal.show();
  }
  ...
  showMortgageManagementModal(username) {
    ...
    const availableForMortgage = allProperties.filter(
      (prop) => !player.isPropertyMortgaged(prop)
    );
                ...
                <div class="mortgage-management-modal">
                              ...
                              availableForMortgage.length === 0
                                ...
                                : `<div class="properties-grid">
                                    ${availableForMortgage
                                      .map((prop) =>
                                        this.renderMortgageableProperty(
                                          prop,
                                          player,
                                          isCurrentPlayer
                                        )
                    ...
                    <div class="mortgage-summary mt-3">
                        <div class="row text-center">
                            ...
                            <div class="col-4">
                                <div class="summary-item">
                                    <div class="summary-value text-danger">$${mortgagedProperties
                                      .reduce(
                                        (sum, prop) =>
                                          sum + prop.unmortgageValue,
                                        0
                                      )
                                      .toLocaleString()}</div>
  ...
  renderMortgageableProperty(property, player, canInteract) {
    ...
    const canMortgage =
      canInteract && !player.hasConstructionsInColorGroup(property.color);

    return `
            <div class="property-card available-property">
                <div class="property-header">
                    <div class="property-name">${property.name}</div>
                    <div class="property-type">${this.getPropertyTypeLabel(
                      property
                    )}</div>
                </div>
                <div class="property-details">
                ...
                <div class="property-actions">
                    <button class="btn btn-success btn-sm" 
                            ...
                            onclick="window.gameInstance.mortgageProperty('${
                              player.username
                            }', '${property.id}')">
            ...
            <div class="property-card mortgaged-property">
                ...
                <div class="property-actions">
                    <button class="btn btn-warning btn-sm" 
                            ...
                            onclick="window.gameInstance.unmortgageProperty('${
                              player.username
                            }', '${property.id}')">
  ...
  getPropertyTypeLabel(property) {
    switch (property.type) {
      case "property":
        return `🏠 ${property.color.toUpperCase()}`;
      case "railroad":
        return "🚂 Ferrocarril";
      case "utility":
        return "⚡ Servicio";
  ...
  mortgageProperty(username, propertyId) {
    ...
    if (player !== this.getCurrentPlayer()) {
      this.showGameMessage(
        "Solo puedes hipotecar propiedades durante tu turno"
      );
      return;
    }
    ...
    if (player.mortgageProperty(property)) {
      const mortgageValue =
        property.mortgage || Math.floor(property.price * 0.5);
      this.showGameMessage(
        `${player.username} hipotecó ${property.name} por $${mortgageValue}`
      );
      this.updatePlayerInfoPanel();
      // Refrescar el modal
  ...
  unmortgageProperty(username, propertyId) {
    ...
    if (player !== this.getCurrentPlayer()) {
      this.showGameMessage(
        "Solo puedes deshipotecar propiedades durante tu turno"
      );
      return;
    }
    ...
    if (player.unmortgageProperty(property)) {
      const paymentAmount = Math.floor(
        (property.mortgage || Math.floor(property.price * 0.5)) * 1.1
      );
      this.showGameMessage(
        `${player.username} deshipotecó ${property.name} por $${paymentAmount}`
      );
      this.updatePlayerInfoPanel();
      // Refrescar el modal
      this.showMortgageManagementModal(username);
