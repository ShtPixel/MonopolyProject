class Player {
    ...
    ownsProperty(propertyId) {
        ...
        return this.properties.some(p => parseInt(p.id) === id) ||
    ...
    getProperty(propertyId) {
        const id = parseInt(propertyId);
        return this.properties.find(p => parseInt(p.id) === id) ||
    ...
    mortgageProperty(property) {
        ...
        if (property.type === 'property' && this.hasConstructionsInColorGroup(property.color)) {
            console.log(`No se puede hipotecar ${property.name}. Primero vende las construcciones del grupo ${property.color}`);
            return false;
        }
        ...
        console.log(`${this.username} hipotecó ${property.name} por $${mortgageValue}`);
        return true;
    }
    ...
    unmortgageProperty(property) {
        ...
        if (this.money < paymentRequired) {
            console.log(`${this.username} no tiene suficiente dinero para deshipotecar ${property.name}. Necesita $${paymentRequired}`);
            return false;
        }
        ...
        console.log(`${this.username} deshipotecó ${property.name} por $${paymentRequired}`);
        return true;
    }
        ...
        this.properties.forEach(prop => {
            ...
            if (this.mortgagedProperties.has(propId)) {
                mortgaged.push({
                    ...
                    unmortgageValue: Math.floor(this.mortgagedProperties.get(propId) * 1.1)
                });
            }
        });
        ...
        this.railroads.forEach(railroad => {
            ...
            if (this.mortgagedProperties.has(railroadId)) {
                mortgaged.push({
                    ...
                    unmortgageValue: Math.floor(this.mortgagedProperties.get(railroadId) * 1.1)
                });
            }
        });
        ...
        this.utilities.forEach(utility => {
            ...
            if (this.mortgagedProperties.has(utilityId)) {
                mortgaged.push({
                    ...
                    unmortgageValue: Math.floor(this.mortgagedProperties.get(utilityId) * 1.1)
                });
            }
        });
    ...
    getFinalScorePropertyValue() {
        ...
        this.properties.forEach(prop => {
            const propId = parseInt(prop.id);
            if (!this.mortgagedProperties.has(propId)) {
                totalValue += prop.price + (prop.houses * (prop.housePrice || 100)) + (prop.hotel ? (prop.housePrice || 100) : 0);
            }
        });
    ...
    getPlayerInfo() {
        return {
            ...
            totalMortgageDebt: Array.from(this.mortgagedProperties.values()).reduce((sum, value) => sum + Math.floor(value * 1.1), 0)
        };
    }
}